...
install in `gaveldrop v0.1.7`, because it has no binaries*. And the fake is a **second** executable,
...
```yaml
      - name: Install gaveldrop
        run: |
          curl -fsSL https://github.com/Dr0drigues/gaveldrop/releases/download/v0.1.7/gaveldrop-v0.1.7-x86_64-unknown-linux-musl.tar.gz \
            | tar -xz -C /tmp
          sudo install /tmp/gaveldrop-v0.1.7-*/gaveldrop /tmp/gaveldrop-v0.1.7-*/gaveldrop-fake /usr/local/bin/
```
